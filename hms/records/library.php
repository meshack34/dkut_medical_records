...
<?php
    ...
    function register($email_id_unsafe, $password_unsafe, $full_name_unsafe, $speciality_unsafe = 'doctor', $table = 'users')
    {
        ...
        $docEmail = secure($email_id_unsafe);
        $password = secure($password_unsafe);
        $specilization = secure($speciality_unsafe);
        $doctorName = ucfirst(secure($full_name_unsafe));

        $sql;

        switch ($table) {
            case 'users':
                $sql = "INSERT INTO $table VALUES ('$docEmail', '$password', '$doctorName');";
                break;
            case 'doctors':
                $sql = "INSERT INTO $table VALUES ('$docEmail', '$password', '$doctorName','$specilization');";
                break;
            case 'clerks':
                $sql = "INSERT INTO $table VALUES ('$docEmail', '$password', '$doctorName');";
                break;
